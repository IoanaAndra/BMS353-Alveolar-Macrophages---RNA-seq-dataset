...
```{r message = FALSE, echo = FALSE, warning=FALSE, fig.dim= c(8,5)}
...
ggplot(readAddHeader, aes(x = Sample, y = rawcount)) + geom_point()

boxplot(colSums(assay(dds)))
```

```{r message = FALSE, echo = FALSE, warning=FALSE, results = c(0,2)}
is_expressed <- assay(dds) >= 5
head(is_expressed)

```
```{r fig.cap="Figure - Bar charts that show number of millions of reads for each of the 12 samples"}
library(dplyr)
library(ggplot2)
sampleinfo %>% mutate(MillionsOfReads = colSums(assay(dds))/1e6) %>% 
  ggplot(aes(x = sampleinfo$run, y = MillionsOfReads)) + geom_col(fill="darkgoldenrod2") + ggtitle ("Number of milliions of reads for each sample")
```
...
```{r message = FALSE, echo = FALSE, warning=FALSE}
#Other attempts (not following the worlflow)
