...
```{r Read tx2gene.csv, tidy = FALSE, collapse = FALSE, message = FALSE, warning=FALSE, echo = FALSE } 
library(readr)
...
```{r echo = FALSE, tidy = FALSE, collapse = FALSE, message = FALSE, warning=FALSE, eval = FALSE}
...
# ERROR FOR object quants. Unable knitting the latest version of Rmd. 
...
download.file("ftp://ftp.ensembl.org/pub/release-91/gtf/mus_musculus/Mus_musculus.GRCm38.91.chr.gtf.gz",destfile = gtf_file) #downloading gtf based on organism of interest
...
mutate(condition = ifelse(name == "N03_AM_Naive","naive",condition)) %>%
mutate(condition= ifelse(name == "R08_AM_Allo24h","post_reperfusion",condition)) %>%
...
library(readr)
library(DESeq2)
sampleinfo_corrected <- read_tsv("meta_data/sampleInfo - Corrected.txt", show_col_types = FALSE)
View(sampleinfo_corrected)
dds <- DESeqDataSetFromTximport(txi, 
...
```{r message=FALSE, echo = FALSE, warning=FALSE, fig.cap="Change the PCA plot with one that shows name labels for each sample with corrected sampleinfo - sampleinfo_corrected"}
library(ggplot2)

plotPCA(vsd,intgroup="condition") + theme_bw() + geom_label(aes(label = sampleinfo_corrected$name)) + ggtitle("title") #same as above but with labels
...
```{r echo = FALSE, eval = FALSE}
de_condition #displays in console 
```
...
```{r echo = FALSE, eval = FALSE}
results(de_condition) #shows baseMean, statistc results, p values, adjusted p values etc
```
```{r}
library(dplyr)
library(tibble)
...
assay(vsd) %>%
  ...
  tibble::rownames_to_column("Gene") %>%
  pivot_longer(-Gene) %>%
  ggplot(aes(x = name, y = value)) + geom_boxplot(fill="lightgreen") + theme(axis.text.x.bottom = element_text(angle=90)) + theme_bw() + ggtitle("Plot")
...
dds_time$time <- as.factor(dds_time$time)
colData(dds)
design(dds_time) <- ~time

de_time <- DESeq(dds)
library(dplyr)
library(tibble)
results_condition <- results(de_time) %>%
  as.data.frame %>%
  rownames_to_column("ENSEMBL")
...
```
Using now time as factor of interest.
...
dds_time$time <- as.factor(dds_time$time)
design(dds_time) <- ~time
de_time <- DESeq(dds_time)

results_time <- results(de_time) %>%
  as.data.frame %>% 
  rownames_to_column("ENSEMBL")  

results_time

# saveRDS("Robjects/results_time.rds")


library(readr)
results_time %>% filter(abs(log2FoldChange) > 1, padj < 0.05) %>% 
  write_csv("0h_vs_24h_DE.csv")
```
...
```{r} 
# library(limma)
# results_p_vs_v <- results(de_time, contrast = c("time","0h","2h")) %>% 
#     as.data.frame %>% 
#   rownames_to_column("ENSEMBL")  
# 
# results_l_vs_v <- results(de_time, contrast = c("time","2h","24h")) %>% 
#     as.data.frame %>% 
#   rownames_to_column("ENSEMBL")  
# 
# venn_data <- data.frame("0h" = results_p_vs_v$padj < 0.05, "24" = results_l_vs_v$padj < 0.05)
# 
# vennDiagram(venn_data)
```


*4. Annotation and visualisation of data
...
```{r}
anno <- AnnotationDbi::select(org.Mm.eg.db,keys=results_condition$ENSEMBL,
...
```
```{r}
write.csv(results_annotated,file="results_condition_DESeq_annotated.csv",row.names=FALSE)
saveRDS(results_annotated, file="Robjects/results_cellType_annotated.rds")
```


```{r fig.cap = "Normalized count in each sample of the gene ENSMUSG00000027750"}
library(DESeq2)
## Read counts. These were produced above
dds <- readRDS("Robjects/dds.rds")
plotCounts(dds, "ENSMUSG00000027750",intgroup = "condition") 
```

```{r fig.cap = "Differential expression which presents p values for difference between conditions"}
library(ggplot2)
results_annotated %>%
  mutate(Significant = padj < 0.05 & abs(log2FoldChange) > 2) %>%
  ggplot(aes(x = log2FoldChange, y = -log10(padj), col=Significant)) + geom_point() + theme_bw() + ggtitle("Differential expression - p values and magnituted - differences between genes")
```
...
vsd <- vst(dds)
pheatmap(assay(vsd)[top_genes,], main = "Gene map")
```
