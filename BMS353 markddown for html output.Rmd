...
This was created only to test some things that work in html output. Image alignment can be done, formatting of text with more control, or add syling (CSS). 
...
! [Alt text] (images\image1macrophages.jpg)
...
DiagrammeR::grViz("digraph {
  ...
  rec1 [label = 'Step 1. Raw data']
  rec2 [label = 'Step 2. Quality assesment']
  rec3 [label =  'Step 3. Differential expression']
  rec4 [label = 'Step 4. Plot data']
...
This can be plotted into bar chart

```{r message = FALSE, echo = FALSE, warning=FALSE}

library(ggplot2)

ggplot(data=readAddHeader) + geom_bar(aes(x=readAddHeader$Sample, fill=readAddHeader$rawcount)) + geom_abline(aes(intercept=0,slope=1,color="Line description"),size=2.5,show_guide=TRUE) + guides(fill = guide_legend(override.aes = list(linetype = 0), title="[TITLE OF FILL]"), 
             color = guide_legend(title="[TITLE OF LINE]")) + scale_color_manual(values="darkorchid4")
```
